...
def plot_color_spaces_4to5():
    ...
    source_poly = Polygon(
        np.column_stack([source_x[:4], source_y[:4]]), alpha=0.2, color="blue"
    )
    ...
    plt.plot(
        display_x, display_y, "-", color="red", linewidth=2, label="5通道显示RGBCX"
    )
    display_poly = Polygon(
        np.column_stack([display_x[:5], display_y[:5]]), alpha=0.2, color="red"
    )
    plt.gca().add_patch(display_poly)

    # 标记RGB点
    plt.scatter(
        [SOURCE_RED[0], SOURCE_GREEN[0], SOURCE_BLUE[0], SOURCE_VIOLET[0]],
...
def visualize_4to5_mapping():
    ...
    alt_mapped_colors = [
        alternative_4to5_conversion(np.array(color)) for color in test_colors
    ]
    ...
    # 将XYZ转换为RGB(用于显示)
    source_rgb_values = [
        np.dot(np.linalg.inv(DISPLAY_RGB_matrix), xyz) for xyz in source_xyz_values
    ]
...
def analyze_conversion_accuracy():
    ...
    for i, color in enumerate(test_colors):
        ...
        results.append({"source": color, "target": display_rgbcx, "error": error})
...
def SOURCErgb_to_xy(rgb: torch.Tensor) -> torch.Tensor:
    ...
    # 将矩阵转换为PyTorch张量
    SOURCE_matrix_tensor = torch.tensor(
        SOURCE_matrix, dtype=rgb.dtype, device=rgb.device
    )
...
def displayrgb_to_xy(rgb: torch.Tensor) -> torch.Tensor:
    ...
    # 将矩阵转换为PyTorch张量
    DISPLAY_matrix_tensor = torch.tensor(
        DISPLAY_matrix, dtype=rgb.dtype, device=rgb.device
    )
...
def XYZ_to_Lab(XYZ: torch.Tensor) -> torch.Tensor:
    ...
    def f(t):
        delta = 6 / 29
        return torch.where(
            t > delta**3, torch.pow(t, 1 / 3), t / (3 * delta**2) + 4 / 29
        )
...
def delta_e_cie2000_torch(Lab1: torch.Tensor, Lab2: torch.Tensor) -> torch.Tensor:
    ...
    delta_H_prime = (
        ...
        * torch.sin(delta_h_prime * (np.pi / 180) / 2)
    )
    ...
    T = (
        1
        - 0.17 * torch.cos((h_prime_mean - 30) * (np.pi / 180))
...
dataloader = DataLoader(
    TensorDataset(train_data), batch_size=256, shuffle=True, pin_memory=True
)
...
for epoch in tqdm(range(num_epochs)):
    ...
    for batch in dataloader:
        ...
        # 计算CIEDE2000色差
        batch_loss = color_difference_torch(
            source_xy, output_xy
        )  # criterion(source_rgb_batch,outputs)#delta_e_cie2000_torch(source_Lab, output_Lab).mean() #color_difference_torch(source_xy, output_xy)
...
def visualize_color_mapping():
    ...
    original_xyz = np.array(
        [np.dot(SOURCE_matrix, rgb) for rgb in original_colors[:, :4]]
    )
    ...
    plt.scatter(
        mapped_xy[:, 0], mapped_xy[:, 1], c="green", alpha=0.5, s=20, label="映射后颜色"
    )

    # 绘制转换对应关系(绘制一些样本点的对应关系，避免过于拥挤)
    for i in range(0, len(original_xy), len(original_xy) // 20):
        plt.plot(
            [original_xy[i, 0], mapped_xy[i, 0]],
...
def main():
    ...
    with open("analysis_results.txt", "w", encoding="utf-8") as f:
        ...
        for i, result in enumerate(results):
            ...
            error = result["error"]
            f.write(
                f"颜色{i + 1}\t({color[0]:.2f}, {color[1]:.2f}, {color[2]:.2f}, {color[3]:.2f})\t"
            )
            f.write(
                f"({target[0]:.2f}, {target[1]:.2f}, {target[2]:.2f}, {target[3]:.2f}, {target[4]:.2f})\t"
            )
            f.write(f"{error:.4f}\n")
